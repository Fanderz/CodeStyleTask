...
public class Mover : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < _point.childCount; i++)
            _childPoints[i] = _point.GetChild(i).GetComponent<Transform>();
    }
    ...
    private void Update()
    {
        ...
        if (transform.position == target.position)
            target.position = GetNexPoint();

        transform.position = Vector3.MoveTowards(transform.position, target.position, _speed * Time.deltaTime);
    }

    private Vector3 GetNexPoint()
    {
        if (_pointIndex == _childPoints.Length)
            _pointIndex = 0;

        return _childPoints[_pointIndex++].transform.position;
    }
}